...
def process_and_split_with_midi(tg_path, midi_path, wav_path, output_dir, item_prefix="bai1", singer_name="Singer1"):
    word_dict_list = []
    ...
    # Thuật toán tìm nốt MIDI khớp với từ (Overlap)
    for word in word_dict_list:
        ...
        if matched_note and max_overlap > 0.05:
    ...
    for i, chunk in enumerate(chunks):
        ...
        metadata_list.append({
            ...
            "ep_types": ep_types,
            "singer": singer_name # <--- ĐÃ UPDATE NHẬN DIỆN CA SĨ
        })

    return metadata_list, list(unique_phones)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="TCSinger Multi-Speaker Batch Preprocessor")
    parser.add_argument('--raw_dir', type=str, required=True, help='Thư mục mẹ chứa các thư mục ca sĩ')
    parser.add_argument('--output_dir', type=str, default='data/processed/tc', help='Thư mục xuất dữ liệu')
    args = parser.parse_args()

    print(f"Bắt đầu quét thư mục Đa ca sĩ: {args.raw_dir} ...")

    # 1. Tìm tất cả file .wav trong toàn bộ thư mục và thư mục con
    wav_files = []
    ...
    if not wav_files:
        print(f"Không tìm thấy file .wav nào trong cấu trúc của {args.raw_dir}")
        exit()

    all_metadata = []
    global_phone_set = set()
    global_singer_dict = {} # Tự động tạo danh sách ca sĩ và ID
    singer_id_counter = 0
    success_count = 0
    ...
    for wav_path in wav_files:
        ...
        # LẤY TÊN CA SĨ DỰA VÀO TÊN THƯ MỤC CHỨA FILE ĐÓ
        singer_name = os.path.basename(os.path.dirname(wav_path))

        # Cập nhật danh sách ca sĩ tự động
        if singer_name not in global_singer_dict:
            global_singer_dict[singer_name] = singer_id_counter
            singer_id_counter += 1

        # Gộp tên ca sĩ vào tên bài để tránh trùng lặp nếu 2 ca sĩ hát cùng 1 bài (VD: DaoLieu_bai1)
        safe_prefix = f"{singer_name}_{base_name}"

        # Lấy đường dẫn thư mục hiện tại để quét TextGrid và MIDI
        dir_path = os.path.dirname(wav_path)
        tg_path = os.path.join(dir_path, f"{base_name}.TextGrid")

        midi_path = os.path.join(dir_path, f"{base_name}.mid")
        if not os.path.exists(midi_path):
            midi_path = os.path.join(dir_path, f"{base_name}.midi")

        if os.path.exists(tg_path) and os.path.exists(midi_path):
            print(f" [V] Đang xử lý: {safe_prefix} (Ca sĩ: {singer_name})")
            try:
                metadata, phone_set = process_and_split_with_midi(
                    ...
                    item_prefix=safe_prefix,
                    singer_name=singer_name # Nạp tên ca sĩ vào thuật toán
                )
                all_metadata.extend(metadata)
                global_phone_set.update(phone_set)
                success_count += 1
            except Exception as e:
                print(f" [!] Lỗi khi xử lý {safe_prefix}: {e}")
        else:
            print(f" [-] Bỏ qua {base_name}: Thiếu file TextGrid hoặc MIDI đi kèm trong thư mục {singer_name}.")

    if success_count > 0:
        # 1. Xuất metadata.json
        metadata_path = os.path.join(args.output_dir, 'metadata.json')
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(all_metadata, f, indent=4, ensure_ascii=False)

        # 2. Xuất phone_set.json
        ...
        # 3. XUẤT TỰ ĐỘNG SPKER_SET.JSON (Không cần vá lỗi BOM nữa)
        spker_path = os.path.join(args.output_dir, 'spker_set.json')
        with open(spker_path, 'w', encoding='utf-8') as f:
            json.dump(global_singer_dict, f, indent=4, ensure_ascii=False)

        print("-" * 50)
        print(f"Hoàn tất xử lý thành công {success_count}/{len(wav_files)} file!")
        print(f"Danh sách ca sĩ nhận diện được: {global_singer_dict}")
        print(f"Dữ liệu đã lưu tại: {os.path.abspath(args.output_dir)}")
    else:
        print("Không có file nào được xử lý thành công. Vui lòng kiểm tra lại cấu trúc thư mục.")